...
interface PreferenceValues {
    time_conflict_value: number;
    campus_conflict_value: number;
    morning_conflict_value: number;
    evening_conflict_value: number;
}
...
interface ArrangementPlan {
    groups: ClassNumberGroup[];
    sum_value: number;
    conflicts: {
        time_conflicts: number;
        campus_conflicts: number;
        morning_conflicts: number;
        evening_conflicts: number;
    };
}
...
class CourseGroupApp {
    ...
    private arrangeCoursesBtn: HTMLElement;
    private preferencesModal: HTMLElement;
    private resultsModal: HTMLElement;
    private confirmBtn: HTMLElement;
    private cancelBtn: HTMLElement;
    private closeResultsBtn: HTMLElement;
    private resultsContainer: HTMLElement;
    
    // 完整课表相关
    private fullScheduleModal: HTMLElement;
    private closeScheduleBtn: HTMLElement;
    private currentWeekNumber: HTMLElement;
    private customWeekInput: HTMLInputElement;
    private prevWeekBtn: HTMLElement;
    private nextWeekBtn: HTMLElement;
    private goWeekBtn: HTMLElement;
    private fullScheduleContent: HTMLElement;
    private currentWeek: number;
    private currentPlan: ClassNumberGroup[] | null;
    ...
    constructor() {
        ...
        this.customWeekInput = document.getElementById('custom-week-input') as HTMLInputElement;
    ...
    // 显示完整课表弹窗
    private showFullScheduleModal(plan: ClassNumberGroup[]): void {
    ...
    // 隐藏完整课表弹窗
    private hideFullScheduleModal(): void {
    ...
    // 生成完整课表
    private generateFullSchedule(): void {
        if (!this.currentPlan) return;
        
        // 创建13节×7天的表格，使用二维数组表示
        const schedule: (ClassNumberGroup[])[][] = Array(13).fill(null).map(() => Array(7).fill(null).map(() => []));
    ...
    // 处理介意值确认
    private handlePreferencesConfirm(): void {
        // 获取用户输入的介意值
        const timeConflictValue = parseInt((document.getElementById('time-conflict') as HTMLInputElement).value) || 0;
        const campusConflictValue = parseInt((document.getElementById('campus-conflict') as HTMLInputElement).value) || 0;
        const morningConflictValue = parseInt((document.getElementById('morning-conflict') as HTMLInputElement).value) || 0;
        const eveningConflictValue = parseInt((document.getElementById('evening-conflict') as HTMLInputElement).value) || 0;
    ...
    // 开始排课
    private arrangeCourses(preferences: PreferenceValues): void {
        // 获取已选课程组的详细信息
        const selectedGroups = Array.from(this.selectedGroupIds)
            .map(id => groupInfo.find(group => group.class_number_group_id === id))
            .filter((group): group is ClassNumberGroup => group !== undefined);
    ...
    // 按course_number分组
    private groupByCourseNumber(groups: ClassNumberGroup[]): Map<string, ClassNumberGroup[]> {
        const result = new Map<string, ClassNumberGroup[]>();
    ...
    // 生成所有可能的选课方案
    private generateAllPlans(groupsByCourseNumber: Map<string, ClassNumberGroup[]>): ClassNumberGroup[][] {
        // 获取所有course_number的列表
        const courseNumbers = Array.from(groupsByCourseNumber.keys());

        // 递归生成所有组合
        const generateCombinations = (index: number): ClassNumberGroup[][] => {
    ...
    // 计算方案的介意值
    private calculatePreferenceScore(plan: ClassNumberGroup[], preferences: PreferenceValues): ArrangementPlan {
        const conflicts = {
            time_conflicts: 0,
            campus_conflicts: 0,
            morning_conflicts: 0,
        ...
        for (let week = 1; week <= 18; week++) {
            // 时间槽映射：存储每天每节课的课程信息
            const timeSlots: Map<string, CourseTimeSlot[]> = new Map();
            // 校区映射：存储每天上午和下午的校区信息
            const campusMap: Map<string, Set<number>> = new Map();
        ...
        // 计算总介意值
        const sum_value = 
            conflicts.time_conflicts * preferences.time_conflict_value +
    ...
    // 检查当前周是否在课程周次范围内
    private isWeekInRange(week: number, weekInfo: WeekInfo): boolean {
        // 检查周次是否在范围内
    ...
    // 获取时间段（上午/下午）
    private getSession(period: number): string {
    ...
    // 显示排课结果
    private displayResults(plans: ArrangementPlan[]): void {
        if (plans.length === 0) {
        ...
        this.resultsContainer.querySelectorAll('.view-schedule-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const planIndex = parseInt((e.target as HTMLElement).dataset.planIndex!);
